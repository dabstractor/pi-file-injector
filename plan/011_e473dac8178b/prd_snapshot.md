...
4. **Inject the entire content** if it fits the remaining context budget; otherwise hand off to §5.5 (paged delivery). No silent truncation in either path. (See §6 for format.) When the token carried a line-range suffix (`:N` / `:N-M`), the content is **sliced to the range first** and the budget decision runs on the slice; the slice pages exactly like a whole file (LR-1, §17.5), and a start past EOF delivers nothing — verbatim token + notify, never an empty block (LR-4, §17.6).
...
**Decision (per text file).** Estimate the file's own cost with the chars-per-token heuristic `fileCost = Math.ceil(content.length / 4)` (O-3). If `remaining === null` (budget unknown) or `fileCost <= PAGED_THRESHOLD * remaining`, inject the whole file inline (§5.1, §6) and subtract `fileCost` from `remaining`. Otherwise page it (below). With a line-range suffix the identical decision runs against the **sliced** content — a range is bounded selection intent, not an overflow override — and a paged slice's directive resumes in file coordinates (`sliceStart + headLines`; LR-1, §17.5). `PAGED_THRESHOLD` defaults to `0.6`: a file that would leave the model less than 40% of remaining context for reasoning trips the page path, even if it technically fits.
...
---  Feature: `#@path:N` / `:N-M` line ranges

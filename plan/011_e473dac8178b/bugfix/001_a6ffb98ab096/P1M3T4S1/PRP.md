...
depends_on: "Nothing in this plan (BUG-004 is independent of BUG-001..003 — all landed — and of BUG-005 — disjoint regions). Requires only the shipped autocomplete provider (file-injector.ts :1933-1985) and the A1 test case (:1006-1050) — both verified present at HEAD."
consumed_by: "P1.M4.T6.S2 (spec/ consistency pass — my optional §14.2 sentence may be folded there instead); P1.M4.T6.S1 (README sweep — NO autocomplete README change from this task; :210/:221 are already accurate)."
supersedes: "plan/…/P1M3T1S1/PRP.md — the SAME work item's PRP under the pre-renumber task tree. This PRP re-anchors it to HEAD (BUG-001..003 landed; baselines re-verified 183/0 + typecheck 0 errors) and is the authoritative one."
---
...
> extended in place (with its blanket assertion NARROWED — the flagged trap) + one optional spec sentence.
> `applyCompletion`, the prefix-level guard, the returned shape, and the registration are all UNCHANGED.
> Failing-test-FIRST discipline. No exported-surface change. No README change.
...
the same original object — while genuine `@<path>` file suggestions still remap to `#@<path>`. The
comment above the map ("pass them through untouched so we don't mangle slash-command suggestions")
becomes literally TRUE. `applyCompletion` behavior is unchanged (it gates on the `prefix` argument, not
on item values — pass-through items already insert fine).
...
- **`file-injector.test.mjs`** — the A1 case (:1006-1050), edited IN PLACE: `fakeCurrent.getSuggestions`
  ...
  is NARROWED to the two `@src/…` items; new asserts pin the `/cmd` pass-through (value untouched,
  strict object identity, no `#@/cmd` fingerprint). All other A1 asserts byte-for-byte unchanged.
- **`spec/14-autocomplete.md`** — OPTIONAL one sentence in §14.2 after "each item value `@<path>` →
  `#@<path>`" (:28): non-`@` (and non-string) item values are passed through untouched.

**Success Definition:**
1. Failing test FIRST: with the extended A1 fake but BEFORE the source fix, A1 fails (`#@/cmd` produced,
   identity assert fails) — proving the test bites.
...
3. `npm run typecheck` → `type-checks clean under --strict (0 errors)` (re-verified green at HEAD today).
4. `applyCompletion` (deterministic `#@` insert + delegation) unchanged — A1's existing apply asserts
   (deterministic replace :1043-1045, cursor position, non-`#@` delegation :1048-1049) stay green untouched.
...
**Pain Points Addressed:** BUG-004 — the defensive remap contradicted its own comment and purpose
("don't mangle slash-command suggestions"), turning every non-`@` value into a fake `#@` token. Latent
today (pi's `@`-queries normally return only `@`-prefixed values) but a correctness/comment-contract
violation waiting for the first mixed result; also a latent crash (`.startsWith` on a non-string value)
that the same guard chain removes.
...
- **Implements the PRD's own Recommendation (§h2.5, BUG-004):** "In the autocomplete item remap, leave
  ...
  inputs; this PRP transcribes them against current verified line anchors at HEAD.
...
- **Kills a latent crash too.** `let v = it.value; … v.startsWith(…)` throws if a misbehaving provider
  returns a non-string value. The guard chain's first check (`typeof it.value !== "string" → return it`)
  removes that hazard in the same edit — no extra scope.
...
- **Bounds the blast radius.** One map body in one closure inside the autocomplete `session_start`
  handler. No exported symbol, no pipeline change, no README churn (the README autocomplete lines
  :210/:221 are already accurate; the changeset sweep is P1.M4.T6.S1/S2).
...
- [ ] The blanket assertion at :1035 is NARROWED (no longer asserts every item is `#@…`).
- [ ] All other A1 asserts (seenLines rewrite :1033, prefix remap :1034, out2 delegation :1039-1040,
      deterministic apply + cursor :1043-1045, non-`#@` apply delegation :1048-1049) unchanged and green.
...
This PRP includes: the current buggy map VERBATIM with its verified line anchor (:1959-1963, mangle at
:1961 — verified at HEAD today) and the drift warning vs. the contract's/arch-doc's older numbers; the
exact replacement map body (guard order + why it is typecheck-safe); the unchanged neighbors enumerated
with anchors (prefix guard, returned shape, applyCompletion internals incl. its prefix-argument gate and
verbatim-value insert); the A1 test structure verbatim (:1006-1050) — the fakeCurrent shape, the
captureHandler + ctx capture pattern, the exact blanket-assertion line to narrow (:1035) and its
replacement, the hoisted-const identity-assert trick, and the mangle fingerprint (`#@/cmd`); the
failing-test-first ordering; the sibling-task boundaries (BUG-005 disjoint; BUG-001..003 landed); the
verified gates and the 183-count invariance; and the docs surface (comment + optional §14.2 sentence +
NO README). The implementer edits two files in two small regions and runs three commands.
...
- file: plan/011_e473dac8178b/bugfix/001_a6ffb98ab096/architecture/spec_ux_bug004_005.md
  why: "§ BUG-004: the provider pipeline walk (registration → trigger match → rewrite → delegate →
        ...
        the '#@' + v branch), the fix-inputs snippet (the guard chain), the A1 test walkthrough, and
        the flagged risk (the blanket every()-assertion trap). § test conventions: captureHandler + fakeCurrent."
  critical: "Its line numbers are PRE-DRIFT (analysis snapshot: map :1904-1908). The committed code has
             drifted ≈ +55 lines (verified at HEAD): registration :1933-1935, comment :1955-1956,
             prefix guard :1957, THE MAP :1959-1963 (mangle :1961), returned shape :1964, applyCompletion
             :1966-1980. ANCHOR BY CONTENT — the parallel BUG-005 task (P1.M3.T5.S1, Planned) edits
             URL_SHAPE_RE (:43) + normalization (~:1660, BEFORE this region) and may shift these numbers
             again. Locate the map by its unique body text:
...
- file: plan/011_e473dac8178b/bugfix/001_a6ffb98ab096/prd_snapshot.md
  why: "§h2.3 Issue 3 / BUG-004: the repro (a value like /cmd → '#@/cmd'), the comment-contract
        violation, and 'Verified with a mocked current provider returning items [@a.ts, @ab.ts, /cmd]
        under prefix @a'. §h2.5 Recommendation: 'leave values that do not start with @ (or #@) untouched.'"
...
- file: file-injector.ts
  ...
  pattern: "The provider's getSuggestions: match `#@<partial>` at cursor (:1940-1941) → rewrite '#'
            to space (:1946-1951) → `inner = await current.getSuggestions(rewrittenLines, …)` (:1953) →
...
- file: file-injector.test.mjs
  why: "A1 at :1006-1050 is THE autocomplete case. captureHandler(\"session_start\") (:1007) reuses the
  ...
  pattern: "Mock pattern (per A1, reuse as-is): `const slot = captureHandler(\"session_start\");` →
            headless no-op checks (:1010-1012) → fakeCurrent (getSuggestions captures seenLines;
            applyCompletion echoes; shouldTriggerFileCompletion false) → `ctx = { cwd: TMPDIR, ui:
            { addAutocompleteProvider: (f) => { providerFactory = f; } } }` (:1023) → `await slot.cb({},
            ctx)` (:1024) → `provider = providerFactory(fakeCurrent)` (:1026) → `await
            provider.getSuggestions([\"Review #@src/\"], 0, len, { signal: { aborted: false } })` (:1032).
            No real pi TUI needed."
  gotcha: "The suite count STAYS 183 — extend A1 in place (no new runCase). If you instead add a
           separate `A1-NONAT` case, you must duplicate your own fake and the count becomes 184; the
           in-place path is simpler and prescriptive here. Do NOT reuse the labels BUG-001/BUG-002/
           BUG-003 (older bugs) or MD-LR3/URL-IMG-EMPTY (landed sibling cases)."

# The optional docs file
- file: spec/14-autocomplete.md
  why: "§14.2 'Implementation (shipped) — line-rewrite reuse (Option 1)' (heading at :17). Its shipped
        paragraph (remap clause at :28) describes 'each item value `@<path>` → `#@<path>`'. Add ONE
        sentence after that clause: non-@ (and non-string) item values are passed through untouched.
        OPTIONAL — may instead be folded into P1.M4.T6.S2 (spec consistency pass)."
  gotcha: "Do NOT touch §14.3 (Non-goal, :46) or anything else. spec/ is the 17-part spec tree (SPEC.md
           is the index)."

# The sibling lane (parallel context — boundary confirmation, NOT a dependency)
- file: plan/011_e473dac8178b/bugfix/001_a6ffb98ab096/tasks.json
  why: "P1.M3.T5.S1 (BUG-005: URL_SHAPE_RE ftp alternative at :43 + normalization-site sync ~:1660 +
        spec/15 §7 correction) is the ONLY other open lane in this milestone. FULLY DISJOINT from this
        task — it does not touch the autocomplete region (:1933-1985), file-injector.test.mjs, or
        spec/14. Its file-injector.ts edits are EARLIER in the file → expect line drift if it lands
        first. Anchor by content."
  critical: "BUG-001 (:475/:1065 renderer), BUG-002 (:1558 injectMarkdown + MD-LR3 case), and BUG-003
             (:946 injectUrl + URL-IMG-EMPTY case) have ALL LANDED (commits 71e9f45, c626928, 21bf4fa,
             fe766bd) — the baselines below are re-verified AT that HEAD."
```

### Current Codebase tree (repo root, salient files)

```bash
pi-file-injector/
├── file-injector.ts            # 1987 lines. ← EDITED: getSuggestions item map (:1959-1963) + comment (:1955-1956) ONLY
├── file-injector.test.mjs      # ← EDITED: A1 case (:1006-1050) — extend fake, narrow :1035, add pass-through asserts
├── import-behavior.test.mjs    # untouched (suite 2 of the npm test chain)
├── relative-imports.test.mjs   # untouched (suite 3)
├── url-injection.test.mjs      # untouched (suite 4; BUG-003's lane, already landed)
├── scripts/typecheck.mjs       # untouched (the --strict gate)
├── package.json                # untouched (npm test = 4-suite && chain)
├── spec/
│   ├── 14-autocomplete.md      # ← OPTIONAL one sentence in §14.2 (remap clause :28)
│   └── …(16 more parts + SPEC.md index)
├── README.md                   # untouched (autocomplete lines :210/:221 already accurate)
└── plan/011_e473dac8178b/bugfix/001_a6ffb98ab096/
    ...
    ├── P1M3T1S1/PRP.md         # SUPERSEDED PRP for this same item (pre-renumber numbering) — read-only reference
    └── P1M3T4S1/               # THIS task — PRP.md + research/
```
...
file-injector.test.mjs    # MODIFIED — A1 case (:1006-1050) IN PLACE: hoist atItem1/atItem2/cmdItem consts;
...
spec/14-autocomplete.md   # OPTIONAL — one sentence in §14.2 after "each item value `@<path>` → `#@<path>`" (:28).
...
```ts
// CRITICAL — LINE DRIFT. The tasks.json contract + arch doc cite :1904-1908/:1906/:1902/:1909/:1911-1925/
//   :1878-1880 — all PRE-DRIFT. Current verified AT HEAD (post BUG-001..003): map :1959-1963 (mangle
//   :1961), comment :1955-1956, prefix guard :1957, returned shape :1964, applyCompletion :1966-1980,
//   registration :1933-1935. The parallel BUG-005 lane edits :43 + ~:1660 (EARLIER in the file) and will
//   shift this region again if it lands first. LOCATE THE MAP BY CONTENT — its body is unique:
//     if (!v.startsWith("#@")) v = v.startsWith("@") ? "#" + v : "#@" + v; // @path → #@path
//   `grep -n '"#" + v' file-injector.ts` finds it in any drift state (verified: :1961 today).
...
//   whether pi declares the field `string` (redundant-but-legal) or loose (required narrowing).
//   The current code compiles `.startsWith` directly, so the declared type is string-compatible; the
//   typeof guard is RUNTIME defense and also removes the latent crash
//   (`.startsWith is not a function` on a non-string).
...
//   description: "" };` in A1, use it in the fake's items array, then assert the found item `=== cmdItem`.
//   The buggy code would return a COPY `{ ...it, value: "#@/cmd" }` — both the value assert AND the
//   identity assert fail pre-fix; post-fix both hold. (The fixed guard returns `it` itself.)
...
//   (helper :173-187 returns {cb: last, all}) — A1's existing `slot.cb` invocations drive the
//   LAST-registered (autocomplete) one; that behavior is already green, keep the calls as-is.

// GOTCHA — suite count invariance. Extending A1 in place adds NO case: expect `Result: 183 passed,
//   0 failed.` after the fix (same as the baseline re-verified at HEAD today). If you chose the
//   separate A1-NONAT case instead, the count becomes 184 — either is permitted, but do not mix both.
...
//   must pass. The test harness loads the REAL committed file via jiti with pi's alias map — no module
//   mocking, only the fakeCurrent provider mock per A1's established pattern.
```
...
provider result shape stays `{ prefix, items }`. The fix only changes WHICH items are copied vs passed
through by reference:

```ts
// BEFORE (file-injector.ts :1959-1963) — the BUG: any value not already '#@' is force-prefixed
// (mangles '/cmd'; also crashes on a non-string value):
...
```yaml
Task 1: EDIT the A1 case — extend the fake (file-injector.test.mjs :1016-1021)
  ...
  - ADD one banner-comment line in the A1 header (:1000-1005) noting the /cmd item pins BUG-004
    pass-through.

Task 2: EDIT the A1 case — narrow the trap + add pass-through asserts (:1035 region)
...
Task 3: RUN — confirm the FAILING test bites (TDD red)
  - `node ./file-injector.test.mjs 2>&1 | grep -A3 'A1 —'` (or full run) → A1 MUST FAIL: the '/cmd'
    assert sees '#@/cmd' (mangle) and/or the identity assert fails (a copy was returned).
...
Task 5: VERIFY gates
  ...
  - `npm test` → the 4-suite chain stays green (file-injector + import-behavior + relative-imports
    + url-injection).

Task 6 (OPTIONAL): spec/14-autocomplete.md §14.2 — one sentence
  - After "each item value `@<path>` → `#@<path>`" (:28), add: "Item values that do not start with
    `@` (or `#@`) — for example a stray slash-command suggestion mixed into an `@` query — are passed
    through untouched, as are non-string values."
  - If skipped, note it for P1.M4.T6.S2 (spec consistency pass) — either is acceptable.
```
...
FILE_EDITS (file-injector.ts):
  ...
  - Comment (:1955-1956): restated to the item-level pass-through (becomes TRUE).
  - UNCHANGED: trigger match/rewrite (:1940-1951), abort/empty guard (:1954), prefix guard (:1957),
...
FILE_EDITS (file-injector.test.mjs):
  - A1 case (:1006-1050) IN PLACE: +3 hoisted item consts; fake items array uses them (+cmdItem);
  ...
  - UNCHANGED: every other A1 assert; every other case; captureHandler helper (:173-187);
...
NO_CHANGES: applyCompletion behavior; prefix-level guard; returned shape; exported surface; README.md
            (:210/:221 already accurate); package.json; scripts/typecheck.mjs; the three other test
            files; all plan/ files. No new files.
```
...
# url-injection), exit 0. (Confirms no cross-suite regression; the landed BUG-001..003 cases intact.)
```
...
let sb = null; const pi = { on: (ev, cb) => { if (ev === "session_start") sb = cb; }, registerMessageRenderer: () => {} };
mod.default(pi);
let factory = null;
...
- [ ] No labels reused (BUG-001/002/003, MD-LR3, URL-IMG-EMPTY off-limits; A1 kept for the in-place extension).
- [ ] No README change; optional spec/14 §14.2 sentence (or explicitly deferred to P1.M4.T6.S2).

### Documentation

- [ ] The source comment now documents the item-level pass-through. [Rides with the work.]
...
- ❌ **Do NOT skip the failing-test-first step.** If the extended A1 passes BEFORE the source fix, your
  test doesn't pin the bug — the fake or the asserts are wrong; fix the test, not the discipline.
- ❌ **Do NOT add a new runCase / reuse landed labels.** Extend A1 IN PLACE (count stays 183). If you
  genuinely prefer a separate case, it MUST be labeled `A1-NONAT` with its own fake (count 184) — never
  `BUG-001`/`BUG-002`/`BUG-003`/`MD-LR3`/`URL-IMG-EMPTY`, and never both variants at once.
- ❌ **Do NOT edit the README or any other spec section.** The README autocomplete lines (:210/:221) are
  already accurate; the changeset sweep is P1.M4.T6.S1/S2. The ONLY optional doc is one sentence in
  spec/14-autocomplete.md §14.2.
...
A surgical, contract-specified fix with anchors re-verified at HEAD today: the buggy map is transcribed
verbatim with its current line numbers (:1959-1963, mangle :1961 — found via the unique-content grep
`"#" + v`, immune to drift), the replacement guard chain is given exactly (with the typecheck-load-bearing
typeof-first ordering and the latent-crash removal as a bonus), the A1 test site is transcribed
(:1006-1050) with THE trap (:1035 blanket assert) explicitly narrowed, the identity-assert trick
(hoisted `cmdItem` + `===`) pins the "original object, untouched" semantics, and the failing-test-first
ordering is enforced as a gate. Baselines re-verified at HEAD: 183/0 primary suite, typecheck 0 errors.
The parallel sibling BUG-005 lane (P1.M3.T5.S1) is confirmed disjoint (URL_SHAPE_RE :43 + normalization
~:1660 + spec/15; it never touches :1933-1985, file-injector.test.mjs, or spec/14). The −1 reserves for
the line drift if BUG-005 lands first (mitigated: every edit anchored by unique content, with the
one-liner grep to relocate the map) and the minor uncertainty in pi's declared item-value type
(typeof-first ordering makes the map compile either way; the typecheck gate confirms). The implementing
agent edits two files in two small regions, demonstrates red→green, and runs three verified commands.
...
But it **does violate explicit PRD acceptance criteria** (§6.3 expanded view; §11 test #34 "Both expand together on ctrl+o") for the most common multi-file use case (`Diff #@a.ts vs #@b.ts`, `#@spec.mdpec.md` with transitive imports, etc.).
...
1. `spec.md` contains `#@api.mdec.md` contains `#@api.md contains `#@api.mdins `#@api.md@api.mdd`; `api.md` contains `#@arch.mdi.md` contains `#@arch.mdontains `#@arch.md `#@arch.mdh.md`; `arch.md` is a leaf.
2. Submit `#@spec.mdit `#@spec.mdpec.md`, press `ctrl+o`.
...
#@bar.mdmd
```
- PRD pseudocode: ` ```foo ` closes the fence; `#@bar.mdpseudocode: ` ```foo ` closes the fence; `#@bar.mdcode: ` ```foo ` closes the fence; `#@bar.md` ```foo ` closes the fence; `#@bar.mdoo ` closes the fence; `#@bar.mdloses the fence; `#@bar.mdthe fence; `#@bar.mdnce; `#@bar.md#@bar.mdmd` is **outside** code → imported (if `bar.md` exists).
- Shipped: ` ```foo ` does **not** close; the fence runs to EOF; `#@bar.mdped: ` ```foo ` does **not** close; the fence runs to EOF; `#@bar.md ```foo ` does **not** close; the fence runs to EOF; `#@bar.mdo ` does **not** close; the fence runs to EOF; `#@bar.mdes **not** close; the fence runs to EOF; `#@bar.mdot** close; the fence runs to EOF; `#@bar.mdlose; the fence runs to EOF; `#@bar.mdthe fence runs to EOF; `#@bar.mdnce runs to EOF; `#@bar.mdns to EOF; `#@bar.mdEOF; `#@bar.md#@bar.mdmd` is **inside** code → exempt (left verbatim).

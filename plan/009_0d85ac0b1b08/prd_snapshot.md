...
- The existing `@file` syntax only auto-reads when passed as a **CLI argument** (`pi @file.txt only auto-reads when passed as a **CLI argument** (`pi @file.txtto-reads when passed as a **CLI argument** (`pi @file.txt when passed as a **CLI argument** (`pi @file.txtssed as a **CLI argument** (`pi @file.txta **CLI argument** (`pi @file.txtargument** (`pi @file.txt** (`pi @file.txt@file.txtt "question"`). It is parsed from argv by `processFileArguments()` *before* the session starts.
- **Inside the interactive editor**, typing `@file.txtde the interactive editor**, typing `@file.txtnteractive editor**, typing `@file.txtve editor**, typing `@file.txtr**, typing `@file.txting `@file.txtle.txt` only triggers path *autocomplete*; on submit the literal text `@file.txtonly triggers path *autocomplete*; on submit the literal text `@file.txtggers path *autocomplete*; on submit the literal text `@file.txtth *autocomplete*; on submit the literal text `@file.txtcomplete*; on submit the literal text `@file.txt*; on submit the literal text `@file.txtbmit the literal text `@file.txt literal text `@file.txt text `@file.txtfile.txt` is sent and the model must call the `read` tool itself.
...
### Solution
A new, dedicated syntax: **`#@<path>`**. It is an **unconditional file-delivery** trigger: whatever file the user names, the model receives all of it. When the user writes `#@filename.txtated syntax: **`#@<path>`**. It is an **unconditional file-delivery** trigger: whatever file the user names, the model receives all of it. When the user writes `#@filename.txt **`#@<path>`**. It is an **unconditional file-delivery** trigger: whatever file the user names, the model receives all of it. When the user writes `#@filename.txt`**. It is an **unconditional file-delivery** trigger: whatever file the user names, the model receives all of it. When the user writes `#@filename.txtn **unconditional file-delivery** trigger: whatever file the user names, the model receives all of it. When the user writes `#@filename.txtional file-delivery** trigger: whatever file the user names, the model receives all of it. When the user writes `#@filename.txtelivery** trigger: whatever file the user names, the model receives all of it. When the user writes `#@filename.txtigger: whatever file the user names, the model receives all of it. When the user writes `#@filename.txtver file the user names, the model receives all of it. When the user writes `#@filename.txt user names, the model receives all of it. When the user writes `#@filename.txt the model receives all of it. When the user writes `#@filename.txteceives all of it. When the user writes `#@filename.txtof it. When the user writes `#@filename.txtthe user writes `#@filename.txttes `#@filename.txtame.txt` anywhere in a prompt and submits, the extension reads the file and delivers it to the model **before** the model sees it. If the file fits the model's remaining context it is delivered whole; if it exceeds the remaining context it is delivered in pages the model reads through the `read` tool (see §5.5). No configuration either way.
...
**Markdown transitive imports.** When a delivered file is markdown (`.md`/`.markdown`), the extension also scans its *contents* for further `#@<path>` directives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.mdown transitive imports.** When a delivered file is markdown (`.md`/`.markdown`), the extension also scans its *contents* for further `#@<path>` directives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.mdnsitive imports.** When a delivered file is markdown (`.md`/`.markdown`), the extension also scans its *contents* for further `#@<path>` directives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.md imports.** When a delivered file is markdown (`.md`/`.markdown`), the extension also scans its *contents* for further `#@<path>` directives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.mds.** When a delivered file is markdown (`.md`/`.markdown`), the extension also scans its *contents* for further `#@<path>` directives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.mden a delivered file is markdown (`.md`/`.markdown`), the extension also scans its *contents* for further `#@<path>` directives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.mdlivered file is markdown (`.md`/`.markdown`), the extension also scans its *contents* for further `#@<path>` directives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.md file is markdown (`.md`/`.markdown`), the extension also scans its *contents* for further `#@<path>` directives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.mds markdown (`.md`/`.markdown`), the extension also scans its *contents* for further `#@<path>` directives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.mdown (`.md`/`.markdown`), the extension also scans its *contents* for further `#@<path>` directives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.mdmd`/`.markdown`), the extension also scans its *contents* for further `#@<path>` directives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.mdarkdown`), the extension also scans its *contents* for further `#@<path>` directives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.md`), the extension also scans its *contents* for further `#@<path>` directives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.md extension also scans its *contents* for further `#@<path>` directives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.mdion also scans its *contents* for further `#@<path>` directives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.mdo scans its *contents* for further `#@<path>` directives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.md its *contents* for further `#@<path>` directives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.mdontents* for further `#@<path>` directives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.md* for further `#@<path>` directives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.mdurther `#@<path>` directives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.md`#@<path>` directives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.mdh>` directives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.mdectives and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.md and delivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.mdlivers those files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.mdthose files too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.mdiles too — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.mdo — recursively, because an imported markdown file is itself scanned. This turns a single `#@spec.mdursively, because an imported markdown file is itself scanned. This turns a single `#@spec.mdy, because an imported markdown file is itself scanned. This turns a single `#@spec.mduse an imported markdown file is itself scanned. This turns a single `#@spec.mdimported markdown file is itself scanned. This turns a single `#@spec.mdd markdown file is itself scanned. This turns a single `#@spec.mdown file is itself scanned. This turns a single `#@spec.mde is itself scanned. This turns a single `#@spec.mdself scanned. This turns a single `#@spec.mdanned. This turns a single `#@spec.mdThis turns a single `#@spec.mdrns a single `#@spec.mdingle `#@spec.md#@spec.mdmd` into "spec.md plus everything spec.md points at," with no extra syntax: the import directive inside a markdown file is the **same** `#@<path>` the user types in a prompt. **Extension shorthand:** a markdown import may omit the `.md`/`.markdown` extension — `#@PRD` resolves to `PRD.md` (or `PRD.markdown`) when no bare `PRD` file exists, because `#@` is a strong import signal and an extensionless name that matches a markdown file after appending `.md`/`.markdown` is treated as an exact match (markdown imports only; top-level tokens stay exact-match). Three guards keep it sane and loop-free: (1) imports resolve **relative to the markdown file's own directory** and absolute/tilde paths are ignored (so a shared doc can't pull arbitrary home/system files), (2) **each absolute path is injected at most once across the entire prompt**, which bounds recursion for free, and (3) `#@` inside fenced or inline code is not an import — code is the escape hatch for a markdown doc that wants to *document* the `#@` syntax. Full spec in §5.6.
...
- **Composable for docs.** `#@spec.mdposable for docs.** `#@spec.md for docs.** `#@spec.mdcs.** `#@spec.md#@spec.mdmd` pulls in everything `spec.md` references with the *same* `#@` directive — spec-and-its-dependencies in one token, loop-safe via dedup (§5.6).
...
3. **Extension shorthand.** When the cleaned token has no file extension (`path.extname(token) === ""` — e.g. `PRD`, `sub/notes`), resolution tries `<exact>.md` then `<exact>.markdown` if the exact path is not an existing regular file: `#@PRD` → `PRD.md` (or `PRD.markdown`). Exact-match wins (a bare `PRD` file beats `PRD.md`); tokens already ending in `.md`/`.markdown` or any other extension are exact-only, so `#` never becomes `PRD.md.md`. Top-level user tokens do **not** get this fallback (exact-match only, §4.4).
...
By default a markdown import **requires** the `#@` prefix (§4.5). Some doc conventions write file references as a bare `@file.mdult a markdown import **requires** the `#@` prefix (§4.5). Some doc conventions write file references as a bare `@file.mdarkdown import **requires** the `#@` prefix (§4.5). Some doc conventions write file references as a bare `@file.md import **requires** the `#@` prefix (§4.5). Some doc conventions write file references as a bare `@file.md **requires** the `#@` prefix (§4.5). Some doc conventions write file references as a bare `@file.mdires** the `#@` prefix (§4.5). Some doc conventions write file references as a bare `@file.mdthe `#@` prefix (§4.5). Some doc conventions write file references as a bare `@file.md` prefix (§4.5). Some doc conventions write file references as a bare `@file.mdx (§4.5). Some doc conventions write file references as a bare `@file.md). Some doc conventions write file references as a bare `@file.md doc conventions write file references as a bare `@file.mdnventions write file references as a bare `@file.mdns write file references as a bare `@file.mde file references as a bare `@file.mdreferences as a bare `@file.mdces as a bare `@file.mda bare `@file.md`@file.mdmd` (no `#`); to support those without making `#@` mandatory inside markdown, the extension exposes one opt-in setting.
...
**Step 2 — claim self.** Add the markdown file's own absolute path to the global `injectedSet` *before* scanning, so a self-import (`notes.md` containing `#@notes.md — claim self.** Add the markdown file's own absolute path to the global `injectedSet` *before* scanning, so a self-import (`notes.md` containing `#@notes.md self.** Add the markdown file's own absolute path to the global `injectedSet` *before* scanning, so a self-import (`notes.md` containing `#@notes.md Add the markdown file's own absolute path to the global `injectedSet` *before* scanning, so a self-import (`notes.md` containing `#@notes.md markdown file's own absolute path to the global `injectedSet` *before* scanning, so a self-import (`notes.md` containing `#@notes.mdn file's own absolute path to the global `injectedSet` *before* scanning, so a self-import (`notes.md` containing `#@notes.md own absolute path to the global `injectedSet` *before* scanning, so a self-import (`notes.md` containing `#@notes.mdolute path to the global `injectedSet` *before* scanning, so a self-import (`notes.md` containing `#@notes.mdth to the global `injectedSet` *before* scanning, so a self-import (`notes.md` containing `#@notes.mde global `injectedSet` *before* scanning, so a self-import (`notes.md` containing `#@notes.md `injectedSet` *before* scanning, so a self-import (`notes.md` containing `#@notes.mdedSet` *before* scanning, so a self-import (`notes.md` containing `#@notes.mdbefore* scanning, so a self-import (`notes.md` containing `#@notes.mdscanning, so a self-import (`notes.md` containing `#@notes.md, so a self-import (`notes.md` containing `#@notes.mdelf-import (`notes.md` containing `#@notes.mdrt (`notes.md` containing `#@notes.mdes.md` containing `#@notes.mdontaining `#@notes.mdg `#@notes.mdes.md`) dedups to verbatim and cannot recurse into itself.

**Step 3 — scan for imports.** Compute the file's **code regions** (fenced blocks + inline code, approximate-CommonMark — see §5.6.1), then run `FILE_INJECT_RE` over the content and **drop any match whose start index lies inside a code region**. For each surviving match, clean the token (§4.3); if empty or if it starts with `/` or `~` → ignore (leave verbatim). **Resolve** the rest via `resolveImportPath(token, dirname(abs), tryMdExt=true)` (§4.5): try the exact path; if it is not an existing regular file **and** the token is extensionless (`path.extname(token) === ""`), try `<exact>.md` then `<exact>.markdown` — first existing regular file wins (`#@PRD` → `PRD.md`). If nothing resolves → ignore (leave verbatim). The scan helper is `async` (it stats candidate paths) and maintains a per-file `localSeen` set, checked alongside the global `injectedSet` **on the resolved abs**: if already in either → leave verbatim; otherwise add it to `localSeen` and record the resolved `abs` as a **resolved import** (encounter order is preserved for the depth-first recursion in Step 5). (Dedup keys on the *resolved* abs, so `#@PRD` and `#` in the same file collapse to one injection. The per-file set stops two imports of the same file within one markdown from both being injected; the global set handles cross-file and self-import dedup.) When `markdownBareAtImports` is on (§4.6), the scan additionally runs `BARE_AT_RE` over the content and unions its matches in. `#@file` is matched once (the bare regex forbids a preceding `#`), and dedup still keys on the resolved abs. **Markers are detected here only to resolve imports — they are never stripped from the content** (see §6.4 for why the extension never modifies delivered text), so no `index`/`prefixLen` bookkeeping is recorded.
...
| `#@nonexistent.txtt.txt` | Token left verbatim; no block; no error. |
| `#@some/dir/` (directory) | Token left verbatim. |
| `text #@a.txtxt #@a.txta.txt more` | File delivered (green `read a.txt` line below the bubble); the prompt is stored **verbatim** — `#@a.txt more` | File delivered (green `read a.txt` line below the bubble); the prompt is stored **verbatim** — `#@a.txt` | File delivered (green `read a.txt` line below the bubble); the prompt is stored **verbatim** — `#@a.txtile delivered (green `read a.txt` line below the bubble); the prompt is stored **verbatim** — `#@a.txtelivered (green `read a.txt` line below the bubble); the prompt is stored **verbatim** — `#@a.txtred (green `read a.txt` line below the bubble); the prompt is stored **verbatim** — `#@a.txtgreen `read a.txt` line below the bubble); the prompt is stored **verbatim** — `#@a.txt `read a.txt` line below the bubble); the prompt is stored **verbatim** — `#@a.txtd a.txt` line below the bubble); the prompt is stored **verbatim** — `#@a.txtxt` line below the bubble); the prompt is stored **verbatim** — `#@a.txtine below the bubble); the prompt is stored **verbatim** — `#@a.txtelow the bubble); the prompt is stored **verbatim** — `#@a.txtthe bubble); the prompt is stored **verbatim** — `#@a.txtubble); the prompt is stored **verbatim** — `#@a.txt); the prompt is stored **verbatim** — `#@a.txte prompt is stored **verbatim** — `#@a.txtmpt is stored **verbatim** — `#@a.txts stored **verbatim** — `#@a.txtred **verbatim** — `#@a.txt*verbatim** — `#@a.txtatim** — `#@a.txt* — `#@a.txt#@a.txtxt` stays in place, so re-open re-triggers (§6.4). |
| Multiple `#@a.txttiple `#@a.txt `#@a.txt.txt #@b.md #@b.md.md` | Both delivered (two `read` lines, in order); notify `2 whole`. |
| Same path twice (`#@a.ts` + `#@./a.ts`, or `#@a.mdr `#@a.md@a.mdd` that imports `a.ts`) | Injected once across the **whole prompt** (shared `injectedSet`, including imports); repeats left verbatim. |
...
| `#@~/notes.mdes.md` | Tilde-expanded; resolved; injected. |
| `## Static table lookup for hostnames.
# See hosts(5) for details.
127.0.0.1        localhost
::1              localhost
192.168.1.161	 seedbox` (absolute) | Resolved; injected (explicit user intent). |
| `#@file.txt.` (trailing period) | Period trimmed → `file.txt`; injected. |
| `(#@file.txtle.txt)` | `(` is non-word → matches; token `file.txt)` → trimmed to `file.txt`; injected. |
...
| `#@spec.mdec.md` that imports `#@api.md that imports `#@api.mdimports `#@api.mds `#@api.mdpi.md` | Both injected: `spec.md` block delivered **verbatim** (import marker `#@api.md | Both injected: `spec.md` block delivered **verbatim** (import marker `#@api.mdh injected: `spec.md` block delivered **verbatim** (import marker `#@api.mdcted: `spec.md` block delivered **verbatim** (import marker `#@api.md`spec.md` block delivered **verbatim** (import marker `#@api.mdmd` block delivered **verbatim** (import marker `#@api.mdock delivered **verbatim** (import marker `#@api.mdlivered **verbatim** (import marker `#@api.mdd **verbatim** (import marker `#@api.mdrbatim** (import marker `#@api.md** (import marker `#@api.mdport marker `#@api.mdarker `#@api.md`#@api.md.md` preserved), then `api.md` block. Notify `2 whole`. |
| Markdown import is itself markdown (`a.md`→`b.md`→`c.md`) | All three injected, pre-order: `a.md`, `b.md`, `c.md`. Each once. |
| Cycle (`a.md`→`b.md`→`a.md`) | `a.md` injected once (claimed before its own scan); `b.md`'s `#@a.mdcle (`a.md`→`b.md`→`a.md`) | `a.md` injected once (claimed before its own scan); `b.md`'s `#@a.md(`a.md`→`b.md`→`a.md`) | `a.md` injected once (claimed before its own scan); `b.md`'s `#@a.mdmd`→`b.md`→`a.md`) | `a.md` injected once (claimed before its own scan); `b.md`'s `#@a.md`b.md`→`a.md`) | `a.md` injected once (claimed before its own scan); `b.md`'s `#@a.mdd`→`a.md`) | `a.md` injected once (claimed before its own scan); `b.md`'s `#@a.mda.md`) | `a.md` injected once (claimed before its own scan); `b.md`'s `#@a.md`) | `a.md` injected once (claimed before its own scan); `b.md`'s `#@a.md `a.md` injected once (claimed before its own scan); `b.md`'s `#@a.mdmd` injected once (claimed before its own scan); `b.md`'s `#@a.mdinjected once (claimed before its own scan); `b.md`'s `#@a.mdcted once (claimed before its own scan); `b.md`'s `#@a.md once (claimed before its own scan); `b.md`'s `#@a.mde (claimed before its own scan); `b.md`'s `#@a.mdlaimed before its own scan); `b.md`'s `#@a.mded before its own scan); `b.md`'s `#@a.mdefore its own scan); `b.md`'s `#@a.mde its own scan); `b.md`'s `#@a.mds own scan); `b.md`'s `#@a.mdn scan); `b.md`'s `#@a.mdan); `b.md`'s `#@a.md `b.md`'s `#@a.mdmd`'s `#@a.mds `#@a.md@a.mdd` left verbatim. No infinite loop. |
| Markdown import with absolute/tilde (`## Static table lookup for hostnames.
# See hosts(5) for details.
127.0.0.1        localhost
::1              localhost
192.168.1.161	 seedbox` inside `a.md`) | Ignored (relative-only); left verbatim as `## Static table lookup for hostnames.
# See hosts(5) for details.
127.0.0.1        localhost
::1              localhost
192.168.1.161	 seedbox` in injected content. |
| `#@path` inside fenced/inline code in `a.md` | Not an import; left verbatim. (Escape hatch for documenting `#@`.) |
| `#@notes.mdes.md` where `notes.md` imports a missing `api.md` | `notes.md` injected (verbatim); `#@api.md where `notes.md` imports a missing `api.md` | `notes.md` injected (verbatim); `#@api.md `notes.md` imports a missing `api.md` | `notes.md` injected (verbatim); `#@api.mds.md` imports a missing `api.md` | `notes.md` injected (verbatim); `#@api.mdimports a missing `api.md` | `notes.md` injected (verbatim); `#@api.mds a missing `api.md` | `notes.md` injected (verbatim); `#@api.mdssing `api.md` | `notes.md` injected (verbatim); `#@api.md`api.md` | `notes.md` injected (verbatim); `#@api.mdd` | `notes.md` injected (verbatim); `#@api.mdnotes.md` injected (verbatim); `#@api.mdmd` injected (verbatim); `#@api.mdjected (verbatim); `#@api.md (verbatim); `#@api.mdatim); `#@api.md `#@api.mdi.md` left verbatim in `notes.md` content (unresolved). |
| `#@notes.mdes.md` where `notes.md` imports a 50 MB `big.log` | `big.log` evaluated against the shared budget; paged if it exceeds remaining. Counted in notify. |
| Markdown import resolves outside cwd (`#@../shared/api.mdt resolves outside cwd (`#@../shared/api.mdde cwd (`#@../shared/api.mdared/api.md` inside `notes.md`) | Allowed (relative to the markdown's dir); injected. |
| Markdown import with a same-named file in BOTH the md's dir AND cwd (`#@b.mdrkdown import with a same-named file in BOTH the md's dir AND cwd (`#@b.mdwn import with a same-named file in BOTH the md's dir AND cwd (`#@b.mdmport with a same-named file in BOTH the md's dir AND cwd (`#@b.mdt with a same-named file in BOTH the md's dir AND cwd (`#@b.mdth a same-named file in BOTH the md's dir AND cwd (`#@b.md same-named file in BOTH the md's dir AND cwd (`#@b.mde-named file in BOTH the md's dir AND cwd (`#@b.mdmed file in BOTH the md's dir AND cwd (`#@b.mdfile in BOTH the md's dir AND cwd (`#@b.md in BOTH the md's dir AND cwd (`#@b.mdBOTH the md's dir AND cwd (`#@b.md the md's dir AND cwd (`#@b.md md's dir AND cwd (`#@b.mds dir AND cwd (`#@b.mdr AND cwd (`#@b.mdD cwd (`#@b.mdd (`#@b.md#@b.mdmd` inside `dir/a.md`; both `dir/b.md` and `./b.md` exist) | `dir/b.md` injected (md's dir wins); cwd-root `./b.md` never chosen. Resolution is file-relative at every depth. |
| Markdown import missing in the md's dir but present under cwd (`#@ghost.mdwn import missing in the md's dir but present under cwd (`#@ghost.mdt missing in the md's dir but present under cwd (`#@ghost.mdg in the md's dir but present under cwd (`#@ghost.md md's dir but present under cwd (`#@ghost.mdr but present under cwd (`#@ghost.mdesent under cwd (`#@ghost.mdder cwd (`#@ghost.md(`#@ghost.mdt.md` inside `dir/a.md`; only `./ghost.md` exists) | Left verbatim (`#@ghost.mdside `dir/a.md`; only `./ghost.md` exists) | Left verbatim (`#@ghost.mdr/a.md`; only `./ghost.md` exists) | Left verbatim (`#@ghost.md only `./ghost.md` exists) | Left verbatim (`#@ghost.md/ghost.md` exists) | Left verbatim (`#@ghost.mdd` exists) | Left verbatim (`#@ghost.mds) | Left verbatim (`#@ghost.mdt verbatim (`#@ghost.mdim (`#@ghost.mdhost.md`); **no** cwd fallback — the cwd copy is never injected. |
...
| Markdown import already extended (`#` in `a.md`; missing) | Exact-only (no `PRD.md.md`); left verbatim. |
| Markdown dedup across shorthand (`#@PRD` + `#` in same file) | Same resolved abs → injected once; second marker left verbatim. |
| Top-level extensionless (`#@PRD` in prompt; only `PRD.md`) | Exact-only at top level; left verbatim. |
| Missing `.md` at top level (`#@nope.mdng `.md` at top level (`#@nope.md` at top level (`#@nope.mdp level (`#@nope.md (`#@nope.mdpe.md`) | Token left verbatim (missing); no scanning. |
| Markdown imports push total over budget | Later files page against the running total; never silently exceed (§5.6.2). |
| `markdownBareAtImports` off (default); `@api.mdkdownBareAtImports` off (default); `@api.mdareAtImports` off (default); `@api.mdmports` off (default); `@api.md` off (default); `@api.md(default); `@api.mdlt); `@api.md@api.mdd` in `a.md` | Bare `@` not matched; left verbatim. Only `#@` imports. |
| `markdownBareAtImports` on; `@api.mdkdownBareAtImports` on; `@api.mdareAtImports` on; `@api.mdmports` on; `@api.md` on; `@api.md`@api.mdmd` in `a.md` (file exists) | Imported (bare-`@`); marker stays verbatim in `a.md`'s block. Same rules as `#@`. |
| `markdownBareAtImports` on; bare `@` in the FIRST imported file (prompt `#@a.mdarkdownBareAtImports` on; bare `@` in the FIRST imported file (prompt `#@a.mdownBareAtImports` on; bare `@` in the FIRST imported file (prompt `#@a.mdareAtImports` on; bare `@` in the FIRST imported file (prompt `#@a.mdtImports` on; bare `@` in the FIRST imported file (prompt `#@a.mdorts` on; bare `@` in the FIRST imported file (prompt `#@a.md` on; bare `@` in the FIRST imported file (prompt `#@a.md; bare `@` in the FIRST imported file (prompt `#@a.mdre `@` in the FIRST imported file (prompt `#@a.md@` in the FIRST imported file (prompt `#@a.mdn the FIRST imported file (prompt `#@a.mde FIRST imported file (prompt `#@a.mdRST imported file (prompt `#@a.mdimported file (prompt `#@a.mdrted file (prompt `#@a.md file (prompt `#@a.mde (prompt `#@a.mdrompt `#@a.mdt `#@a.md@a.mdd`; `a.md` contains `@b.md`a.md` contains `@b.mdd` contains `@b.mdontains `@b.mdins `@b.md`@b.mdmd`) | `b.md` imported — the first file is not special-cased; bare-`@` is honored at depth 0→1 just like deeper levels. No `#@` required inside `a.md`. |
| `markdownBareAtImports` on; bare-`@` chain across depths (prompt `#@a.mdarkdownBareAtImports` on; bare-`@` chain across depths (prompt `#@a.mdownBareAtImports` on; bare-`@` chain across depths (prompt `#@a.mdareAtImports` on; bare-`@` chain across depths (prompt `#@a.mdtImports` on; bare-`@` chain across depths (prompt `#@a.mdorts` on; bare-`@` chain across depths (prompt `#@a.md` on; bare-`@` chain across depths (prompt `#@a.md; bare-`@` chain across depths (prompt `#@a.mdre-`@` chain across depths (prompt `#@a.md@` chain across depths (prompt `#@a.mdhain across depths (prompt `#@a.md across depths (prompt `#@a.mdoss depths (prompt `#@a.mddepths (prompt `#@a.mdhs (prompt `#@a.mdprompt `#@a.mdpt `#@a.md#@a.mdmd`; `a.md`→`@b.md `a.md`→`@b.mdmd`→`@b.md`@b.mdmd`→`@c.md`@c.mdmd`) | `a.md`, `b.md`, `c.md` all injected; every level honored (no asymmetry between first and deeper files). |
| `#@api.mdpi.md` with the option on | Matched once by `#@` (bare regex skips a `#`-preceded `@`); never double-matched. |
| `@username` in prose (option on; no `username.md`) | Not resolved → left verbatim. Prose imports only if it names a real file. |
| `@api.mdi.md` at the top level (option on) | Unaffected — top-level is `#@`-only; Pi's normal `@` behavior. |
...
| 10 | tilde | `Read #@~/notes.mdde | `Read #@~/notes.md #@~/notes.md.md` | Expanded; injected. |
...
| 15 | md import | `notes.md` containing `#@api.md md import | `notes.md` containing `#@api.mdport | `notes.md` containing `#@api.md `notes.md` containing `#@api.mds.md` containing `#@api.mdcontaining `#@api.mdning `#@api.md#@api.mdmd`; `#@notes.mdnotes.md` | `notes.md` block delivered verbatim (marker `#@api.mdmd` | `notes.md` block delivered verbatim (marker `#@api.md`notes.md` block delivered verbatim (marker `#@api.md.md` block delivered verbatim (marker `#@api.mdlock delivered verbatim (marker `#@api.mdelivered verbatim (marker `#@api.mded verbatim (marker `#@api.mdbatim (marker `#@api.md(marker `#@api.mdr `#@api.mdpi.md` preserved) then `api.md` block; notify `2 whole`; no `read` calls. |
| 16 | md code-exempt | `notes.md` with `` `#@example.ts` `` in a fenced block + a real `#@api.mdn a fenced block + a real `#@api.mdnced block + a real `#@api.mdlock + a real `#@api.md a real `#@api.mdl `#@api.mdpi.md` | Only `api.md` imported; `#@example.ts` left verbatim in code. |
| 17 | md cycle | `a.md`→`#@b.md | md cycle | `a.md`→`#@b.mdd cycle | `a.md`→`#@b.mdcle | `a.md`→`#@b.md| `a.md`→`#@b.md.md`→`#@b.md→`#@b.mdb.md`, `b.md`→`#@a.md`, `b.md`→`#@a.mdb.md`→`#@a.md`→`#@a.md@a.mdd`; `#@a.md`#@a.md.md` | `a.md` + `b.md` injected once each; `b.md`'s `#@a.md | `a.md` + `b.md` injected once each; `b.md`'s `#@a.mda.md` + `b.md` injected once each; `b.md`'s `#@a.md` + `b.md` injected once each; `b.md`'s `#@a.md`b.md` injected once each; `b.md`'s `#@a.mdd` injected once each; `b.md`'s `#@a.mdnjected once each; `b.md`'s `#@a.mdted once each; `b.md`'s `#@a.mdonce each; `b.md`'s `#@a.md each; `b.md`'s `#@a.mdh; `b.md`'s `#@a.mdb.md`'s `#@a.md`'s `#@a.md`#@a.md.md` verbatim; no loop; notify `2 whole`. |
| 18 | md abs rejected | `notes.md` with `## Static table lookup for hostnames.
# See hosts(5) for details.
127.0.0.1        localhost
::1              localhost
192.168.1.161	 seedbox`; `#@notes.mdtes.md` | `/etc/hosts` not imported; marker verbatim; only `notes.md` injected. |
| 19 | md relative base | `sub/notes.md` imports `api.md` (sibling); `#@sub/notes.mdlative base | `sub/notes.md` imports `api.md` (sibling); `#@sub/notes.md| `sub/notes.md` imports `api.md` (sibling); `#@sub/notes.md.md` imports `api.md` (sibling); `#@sub/notes.md `api.md` (sibling); `#@sub/notes.mdibling); `#@sub/notes.mdsub/notes.md` | `api.md` resolved as `sub/api.md` (relative to the md's dir), injected. |
| 20 | budget total | `#@a.md | budget total | `#@a.mdudget total | `#@a.mdt total | `#@a.mdtal | `#@a.md| `#@a.md@a.mdd` importing 3 files + `#@big.log` (huge) | Imports share budget with top-level; `big.log` pages when total exceeds remaining; notify counts all delivered files. |
| 21 | md ext-shorthand | `notes.md` imports `#@api` (`api.md` exists); `#@notes.mdmd` exists); `#@notes.mdts); `#@notes.mdnotes.md` | `notes.md` block (verbatim; `#@api` preserved) then `api.md` block; notify `2 whole`. |
| 22 | md ext exact-wins | `notes.md` imports `#@readme` where both `readme` and `readme.md` exist; `#@notes.mdboth `readme` and `readme.md` exist; `#@notes.mdadme` and `readme.md` exist; `#@notes.mdd `readme.md` exist; `#@notes.mde.md` exist; `#@notes.mdist; `#@notes.mdnotes.md` | Bare `readme` (exact) injected, not `readme.md`; notify `2 whole`. |
| 23 | md ext `.markdown` | `#@api` where only `api.markdown` exists; `#@notes.mdonly `api.markdown` exists; `#@notes.mdi.markdown` exists; `#@notes.mdwn` exists; `#@notes.mdts; `#@notes.mdotes.md` | Resolves to `api.markdown`; injected. |
| 24 | top-level no fallback | `#@PRD` at top level, only `PRD.md` exists | Left verbatim (exact-only at top level); no injection. |
| 25 | bare-`@` off (default) | `notes.md` with `@api.md bare-`@` off (default) | `notes.md` with `@api.md`@` off (default) | `notes.md` with `@api.mdf (default) | `notes.md` with `@api.mdault) | `notes.md` with `@api.md| `notes.md` with `@api.mdes.md` with `@api.md with `@api.md`@api.mdmd` (exists); `#@notes.mdsts); `#@notes.md@notes.mdd` | `api.md` **not** imported (default); only `notes.md`; `@api.mdapi.md` **not** imported (default); only `notes.md`; `@api.md` **not** imported (default); only `notes.md`; `@api.mdt** imported (default); only `notes.md`; `@api.mdported (default); only `notes.md`; `@api.md (default); only `notes.md`; `@api.mdult); only `notes.md`; `@api.mdonly `notes.md`; `@api.mdnotes.md`; `@api.mdmd`; `@api.md@api.mdd` left verbatim. |
| 26 | bare-`@` on | config `markdownBareAtImports:true`; `notes.md` with `@api.md bare-`@` on | config `markdownBareAtImports:true`; `notes.md` with `@api.md`@` on | config `markdownBareAtImports:true`; `notes.md` with `@api.md | config `markdownBareAtImports:true`; `notes.md` with `@api.mdfig `markdownBareAtImports:true`; `notes.md` with `@api.mdarkdownBareAtImports:true`; `notes.md` with `@api.mdnBareAtImports:true`; `notes.md` with `@api.mdtImports:true`; `notes.md` with `@api.mdts:true`; `notes.md` with `@api.mde`; `notes.md` with `@api.mdotes.md` with `@api.mdd` with `@api.mdh `@api.mdi.md` (exists); `#@notes.mdxists); `#@notes.md`#@notes.md.md` | `notes.md` block (verbatim; `@api.md `notes.md` block (verbatim; `@api.mds.md` block (verbatim; `@api.mdblock (verbatim; `@api.md(verbatim; `@api.mdtim; `@api.md@api.mdd` preserved) then `api.md` block; notify `2 whole`. |
| 27 | bare-`@` on, `#@` still works | config on; `notes.md` with `#@api.md bare-`@` on, `#@` still works | config on; `notes.md` with `#@api.md`@` on, `#@` still works | config on; `notes.md` with `#@api.md, `#@` still works | config on; `notes.md` with `#@api.md still works | config on; `notes.md` with `#@api.md works | config on; `notes.md` with `#@api.md | config on; `notes.md` with `#@api.mdfig on; `notes.md` with `#@api.md; `notes.md` with `#@api.mdes.md` with `#@api.md with `#@api.md`#@api.md.md`; `#@notes.md@notes.mdd` | `#@api.md#@api.mdmd` matched once, injected once; notify `2 whole`. |
| 28 | bare-`@` on, top-level unaffected | config on; prompt `#@notes.mdare-`@` on, top-level unaffected | config on; prompt `#@notes.mdon, top-level unaffected | config on; prompt `#@notes.mdlevel unaffected | config on; prompt `#@notes.mdaffected | config on; prompt `#@notes.md | config on; prompt `#@notes.mdg on; prompt `#@notes.mdompt `#@notes.mdnotes.md` (notes imports `@x.mds.md` (notes imports `@x.md` (notes imports `@x.mdotes imports `@x.md imports `@x.mdorts `@x.md `@x.md.md`); also type `@other.mdlso type `@other.md `@other.md.md` in prompt | `@other.mdprompt | `@other.md `@other.md.md` at top level left as Pi's `@` behavior (not injected); only the `#@` chain runs. |
| 29 | bare-`@` via settings.json | `markdownBareAtImports:true` under `fileInjector` in settings.json; `notes.md` with `@api.md bare-`@` via settings.json | `markdownBareAtImports:true` under `fileInjector` in settings.json; `notes.md` with `@api.md`@` via settings.json | `markdownBareAtImports:true` under `fileInjector` in settings.json; `notes.md` with `@api.mda settings.json | `markdownBareAtImports:true` under `fileInjector` in settings.json; `notes.md` with `@api.mdings.json | `markdownBareAtImports:true` under `fileInjector` in settings.json; `notes.md` with `@api.mdson | `markdownBareAtImports:true` under `fileInjector` in settings.json; `notes.md` with `@api.md`markdownBareAtImports:true` under `fileInjector` in settings.json; `notes.md` with `@api.mdownBareAtImports:true` under `fileInjector` in settings.json; `notes.md` with `@api.mdeAtImports:true` under `fileInjector` in settings.json; `notes.md` with `@api.mdorts:true` under `fileInjector` in settings.json; `notes.md` with `@api.mdrue` under `fileInjector` in settings.json; `notes.md` with `@api.mdnder `fileInjector` in settings.json; `notes.md` with `@api.mdfileInjector` in settings.json; `notes.md` with `@api.mdjector` in settings.json; `notes.md` with `@api.md` in settings.json; `notes.md` with `@api.mdettings.json; `notes.md` with `@api.mds.json; `notes.md` with `@api.md; `notes.md` with `@api.mdes.md` with `@api.md with `@api.md`@api.mdmd` (exists); `#@notes.mdsts); `#@notes.md@notes.mdd` | Same as #26 but via the settings.json key: `api.md` imported (bare), notify `2 whole`. |
| 30 | md relative disambiguation | `dir/a.md` imports `#@b.md | md relative disambiguation | `dir/a.md` imports `#@b.mdd relative disambiguation | `dir/a.md` imports `#@b.mdlative disambiguation | `dir/a.md` imports `#@b.mdve disambiguation | `dir/a.md` imports `#@b.mdisambiguation | `dir/a.md` imports `#@b.mdbiguation | `dir/a.md` imports `#@b.mdation | `dir/a.md` imports `#@b.mdn | `dir/a.md` imports `#@b.md`dir/a.md` imports `#@b.md/a.md` imports `#@b.mdd` imports `#@b.mdmports `#@b.mdts `#@b.md#@b.mdmd`; **both** `dir/b.md` and `./b.md` exist; `#@dir/a.mdoth** `dir/b.md` and `./b.md` exist; `#@dir/a.mdir/b.md` and `./b.md` exist; `#@dir/a.md and `./b.md` exist; `#@dir/a.mdb.md` exist; `#@dir/a.mdist; `#@dir/a.mddir/a.md` | `dir/b.md` injected (the md's dir wins); cwd-root `./b.md` has zero blocks. Proves resolution is file-relative, not cwd-relative. |
| 31 | md relative, deep + cwd-indep. | `directory/otherdir/some/file.md` imports `#@file2.mdd relative, deep + cwd-indep. | `directory/otherdir/some/file.md` imports `#@file2.mdve, deep + cwd-indep. | `directory/otherdir/some/file.md` imports `#@file2.md + cwd-indep. | `directory/otherdir/some/file.md` imports `#@file2.mdndep. | `directory/otherdir/some/file.md` imports `#@file2.md`directory/otherdir/some/file.md` imports `#@file2.mdry/otherdir/some/file.md` imports `#@file2.mddir/some/file.md` imports `#@file2.md/file.md` imports `#@file2.md` imports `#@file2.mds `#@file2.mde2.md`; only `…/some/file2.md` exists; also a stray `./file2.md`; `#@directory/otherdir/some/file.mdexists; also a stray `./file2.md`; `#@directory/otherdir/some/file.mdd`; `#@directory/otherdir/some/file.mdfile.md` | `…/some/file2.md` injected (the importing file's dir), never `./file2.md`. |
| 32 | bare-`@` first-file + chain | config on; prompt `#@a.md | bare-`@` first-file + chain | config on; prompt `#@a.mdare-`@` first-file + chain | config on; prompt `#@a.md`@` first-file + chain | config on; prompt `#@a.mdfirst-file + chain | config on; prompt `#@a.mdt-file + chain | config on; prompt `#@a.mdle + chain | config on; prompt `#@a.md chain | config on; prompt `#@a.mdin | config on; prompt `#@a.md config on; prompt `#@a.mdfig on; prompt `#@a.mdon; prompt `#@a.mdprompt `#@a.mdpt `#@a.md#@a.mdmd`; `a.md`→`@b.md `a.md`→`@b.mdmd`→`@b.md`@b.mdmd`→`@c.md`@c.mdmd` (all bare `@`) | `a.md`, `b.md`, `c.md` all injected; the first imported file's bare-`@` is honored (no asymmetry vs. deeper files); notify `3 whole`. |
...
There is still no configuration *required*, and no knobs for the things that should just work: the inline-vs-paged decision is computed from the active model's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mds still no configuration *required*, and no knobs for the things that should just work: the inline-vs-paged decision is computed from the active model's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.md no configuration *required*, and no knobs for the things that should just work: the inline-vs-paged decision is computed from the active model's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdfiguration *required*, and no knobs for the things that should just work: the inline-vs-paged decision is computed from the active model's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdion *required*, and no knobs for the things that should just work: the inline-vs-paged decision is computed from the active model's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdquired*, and no knobs for the things that should just work: the inline-vs-paged decision is computed from the active model's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.md, and no knobs for the things that should just work: the inline-vs-paged decision is computed from the active model's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdo knobs for the things that should just work: the inline-vs-paged decision is computed from the active model's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.md for the things that should just work: the inline-vs-paged decision is computed from the active model's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mde things that should just work: the inline-vs-paged decision is computed from the active model's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mds that should just work: the inline-vs-paged decision is computed from the active model's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdshould just work: the inline-vs-paged decision is computed from the active model's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdjust work: the inline-vs-paged decision is computed from the active model's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdrk: the inline-vs-paged decision is computed from the active model's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.md inline-vs-paged decision is computed from the active model's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.md-vs-paged decision is computed from the active model's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mded decision is computed from the active model's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdsion is computed from the active model's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.md computed from the active model's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mded from the active model's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.md the active model's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdtive model's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mddel's context window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdontext window and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdwindow and the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdand the current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.md current usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdt usage estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.md estimate (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdte (§5.5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.md5), and there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.md there are no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdare no toggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdtoggles for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.md for format, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdrmat, image handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdmage handling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdndling, paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.md paging, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.md, or the context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mde context budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdxt budget — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdet — those stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdose stay derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdy derived/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mded/fixed. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdd. The one opt-in is `markdownBareAtImports` (§4.6): a bare `@file.mdone opt-in is `markdownBareAtImports` (§4.6): a bare `@file.md-in is `markdownBareAtImports` (§4.6): a bare `@file.md`markdownBareAtImports` (§4.6): a bare `@file.mdwnBareAtImports` (§4.6): a bare `@file.mdtImports` (§4.6): a bare `@file.mds` (§4.6): a bare `@file.md6): a bare `@file.mdare `@file.mdile.md` is a widespread doc convention, and forcing `#@` inside markdown would fight existing docs. It is opt-in (default off) precisely so the default stays zero-setup and unambiguous — `#@` remains the only thing that ever triggers injection at the prompt, and bare-`@` matching never escapes markdown content. Knobs for anything else would reintroduce the complexity the user asked to remove.
...
`#@` already delivers a whole file; markdown files are the one format that commonly *references other files by path* in-band. Letting `#@spec.mdready delivers a whole file; markdown files are the one format that commonly *references other files by path* in-band. Letting `#@spec.mdelivers a whole file; markdown files are the one format that commonly *references other files by path* in-band. Letting `#@spec.md a whole file; markdown files are the one format that commonly *references other files by path* in-band. Letting `#@spec.mde file; markdown files are the one format that commonly *references other files by path* in-band. Letting `#@spec.md markdown files are the one format that commonly *references other files by path* in-band. Letting `#@spec.mdwn files are the one format that commonly *references other files by path* in-band. Letting `#@spec.mds are the one format that commonly *references other files by path* in-band. Letting `#@spec.mdhe one format that commonly *references other files by path* in-band. Letting `#@spec.mdformat that commonly *references other files by path* in-band. Letting `#@spec.mdthat commonly *references other files by path* in-band. Letting `#@spec.mdmmonly *references other files by path* in-band. Letting `#@spec.md*references other files by path* in-band. Letting `#@spec.mdnces other files by path* in-band. Letting `#@spec.mdher files by path* in-band. Letting `#@spec.mdes by path* in-band. Letting `#@spec.mdath* in-band. Letting `#@spec.md-band. Letting `#@spec.mdLetting `#@spec.md `#@spec.mdc.md` pull in everything `spec.md` points at matches user intent ("give me the spec and its dependencies") with **no new syntax** — the import directive is the same `#@<path>`.
...
**Why extension shorthand, and why markdown-only.** Markdown imports are authored in files where there is no live path completion (§14 is prompt-only), so a bare `#@PRD` is a natural way to reference the `PRD.md` doc — `#@` is a strong enough import signal that an extensionless name matching a markdown file after appending `.md`/`.markdown` should be treated as an exact match. The fallback is deliberately scoped to markdown imports and to *extensionless* tokens: exact-match always wins (a bare `PRD` file beats `PRD.md`), an explicit `#` never becomes `PRD.md.md`, and top-level prompt tokens stay exact-only (the user has autocomplete there). Dedup keys on the resolved abs, so `#@PRD` and `#` in the same file inject once.
